...
const char * const BITCOIN_CONF_FILENAME = "bandexcoin.conf";
...
#else
    const char* pszModule = "bandexcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bandexcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bandexcoin";
#else
    // Unix
    return pathRet / ".bandexcoin";
#endif
#endif
}
